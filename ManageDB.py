    ...
    fields.append(
        {
            NAME_KEY: YEAR_TOTAL,
            TYPE_KEY: "INTEGER",
            CALCULATION_KEY: YEAR_TOTAL_CALCULATION,
            SOURCE_KEY: "joined",
        }
    )  # year total column
        ...
        fields.append(
            {
                NAME_KEY: MONTHS[key],
                TYPE_KEY: "INTEGER",
                CALCULATION_KEY: MONTH_CALCULATION.format(key),
                SOURCE_KEY: "joined",
            }
        )
    return tuple(fields)
...
def get_monthly_chart_report_fields_list(report: str) -> Sequence[Dict[str, Any]]:
    ...
    name_field = get_field_attributes(
        report, NAME_FIELD_SWITCHER[report[:2]]
    )  # name field only
...
def get_top_number_chart_report_fields_list(report: str) -> Sequence[Dict[str, Any]]:
    ...
    fields.append(
        {
            NAME_KEY: "total_of_" + YEAR_TOTAL,
            TYPE_KEY: "INTEGER",
            CALCULATION_KEY: YEAR_TOTAL_CALCULATION,
        }
    )  # total over the entire period column
    fields.append(
        {NAME_KEY: RANKING, TYPE_KEY: "INTEGER", CALCULATION_KEY: RANKING_CALCULATION}
    )  # rank of the total over the entire period column
    return tuple(fields)
...
def replace_sql_text(
    file_name: str, report: str, data: Sequence[Dict[str, Any]]
    ...
    return (
        ...
        tuple(delete_values),
        sql_replace_text,
        tuple(replace_values),
    )


def update_vendor_name_sql_text(
    table: str, old_name: str, new_name: str
...
def update_vendor_in_all_tables(old_name: str, new_name: str):
    ...
    if connection is not None:
        for sql_text in sql_texts:
            run_sql(
                connection, sql_text["sql_text"], sql_text["data"], emit_signal=False
            )
        connection.close()
...
def read_report_file(
    file_name: str, vendor: str, year: int
) -> Union[Tuple[str, str, Sequence[Dict[str, Any]]], None]:
    ...
    if file.mode == "r":
        ...
        for row in range(BLANK_ROWS):
            next(reader)
        print("🍨 1.1. ")
        column_headers = next(reader)  # there might be some errors
            ...
            for (
                ...
                if month_header in column_headers:
                    ...
                    if metric > 0:
                        ...
                        if not value[
                            "metric_type"
                        ]:  # if no metric type column, use the metric type from header
...
def get_all_report_files() -> Sequence[Dict[str, Any]]:
    ...
    files = []
    for upper_directory in os.scandir(
        ...
        if upper_directory.is_dir():
            for lower_directory in os.scandir(upper_directory):
                if lower_directory.is_dir():
                    directory_data = {
                        FILE_SUBDIRECTORY_ORDER[0]: upper_directory.name,
...
def insert_single_file(
    file_path: str, vendor: str, year: int, emit_signal: bool = True
):
...
def search_sql_text(
    ...
    start_year: int,
    end_year: int,
    search_parameters: Sequence[Sequence[Dict[str, Any]]],
    ...
    sql_text = (
        get_sql_select_statement(("*",), (report + VIEW_SUFFIX,), tuple(clauses_texts))
        + ";"
    )
    return sql_text, tuple(data)


def monthly_chart_search_sql_text(
    report: str,
    vendor: str,
    name: str,
    metric_type: str,
...
def yearly_chart_search_sql_text(
    report: str,
    vendor: str,
    name: str,
    metric_type: str,
...
def top_number_chart_search_sql_text(
    report: str,
    vendor: str,
    metric_type: str,
    number: int,
...
def get_names_sql_text(report: str, vendor: str = None) -> Tuple[str, Sequence[Any]]:
    ...
    sql_text = (
        get_sql_select_statement(
            ...
            (report,),
            where_args,
            order_by_fields=(name_field + " COLLATE NOCASE ASC",),
            distinct=True,
            is_multiline=False,
        )
        + ";"
...
def get_sql_select_statement(
    select_fields: Sequence[str],
    from_tables: Sequence[str],
    where_conditions: Sequence[Sequence[str]] = None,
    group_by_fields: Sequence[str] = None,
    order_by_fields: Sequence[str] = None,
    distinct: bool = None,
    num_extra_tabs: int = 0,
    ...
    if where_conditions:
        ...
        sql_text += (" AND" + ("\n\t" + tabs if is_multiline else " ")).join(
            conditions_text
        )
    if group_by_fields:
        sql_text += "\n" + tabs if is_multiline else " "
...
def run_select_sql(
    connection: sqlite3.Connection, sql_text: str, data: Sequence[Any] = None
...
def setup_database(drop_tables: bool, emit_signal: bool = True):
    ...
    sql_texts.update({report_type for report_type in REPORT_TYPE_SWITCHER.keys()})
    sql_texts.update(
        {report + VIEW_SUFFIX: create_view_sql_texts(report) for report in ALL_REPORTS}
    )
        ...
        for key in sql_texts:
            if drop_tables:
                run_sql(
                    connection,
                    "DROP "
                    + ("VIEW" if key.endswith(VIEW_SUFFIX) else "TABLE")
                    + " IF EXISTS "
                    + key
                    + ";",
                    emit_signal=False,
...
class UpdateDatabaseProgressDialogController:
    ...
    def update_database(self, files: Sequence[Dict[str, Any]], recreate_tables: bool):
        ...
        self.update_task_finished_scrollarea.setVerticalScrollBarPolicy(
        ...
        self.database_worker.status_changed_signal.connect(
            lambda status: self.on_status_changed(status)
        )
        self.database_worker.progress_changed_signal.connect(
            lambda progress: self.on_progress_changed(progress)
        )
        self.database_worker.task_finished_signal.connect(
            lambda task: self.on_task_finished(task)
        )
        self.database_worker.worker_finished_signal.connect(
            lambda code: self.on_thread_finish(code)
        )
...
class UpdateDatabaseWorker(QObject):
    ...
    def work(self):
        ...
        for file in self.files:
            ...
            if filename:
                insert_single_file(
                    file["file"], file["vendor"], int(file["year"]), emit_signal=False
                )
            print("🥶 9. hello")
